...
def introduction():
    st.sidebar.markdown('---')
    st.sidebar.image(Image.open('inference/data/src.jpg'))
    ...
    st.markdown("""
                ...
                通过视频监测，实时准确计算通道双向的进出人数（断面式）；通过综合统计一个逻辑封闭区域所有进入及离开通道的人数，准确计算任何时段该封闭区域内的人数（断面式）；用户可以方便地设定视频中通道方位和方向（断面式）；能够统计视频画面中用户指定任意区域内的人数（区域式）；能处理多人同时通过检测口的复杂情况，能适应现场复杂的背景环境；能够对不同角度的视频画面进行准确统计；提供灵活可靠的数据传输功能，实时向后端发送人数统计数据和记录的视频图像。
